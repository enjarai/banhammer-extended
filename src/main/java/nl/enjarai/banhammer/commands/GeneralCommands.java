...
public class GeneralCommands {
    public static void register() {
        CommandRegistrationCallback.EVENT.register((dispatcher, dedicated) -> {
            ...
            dispatcher.register(literal("history")
                    .requires(Permissions.require("banhammer.history", 3))
                    .then(playerArgument("player")
                            .executes(GeneralCommands::historyCommand)
                    )
            );
            });
        }
    ...
    private static int seenCommand(CommandContext<ServerCommandSource> context) {
        ...
        if (BanHammerMod.SERVER.getPlayerManager().getPlayer(player.uuid) == null) {
            ...
            if (entry == null) {
                ...
                placeholders.put("time", new LiteralText(Helpers.getFormattedDuration(entry.time, true)));
    ...
    private static int historyCommand(CommandContext<ServerCommandSource> context) {
        String playerNameOrIp = context.getArgument("player", String.class);
        BHPlayerData player = Helpers.lookupPlayerDataNoPunishment(playerNameOrIp);
        ...
        for (Text message : player.getFormattedPunishmentHistory()) {
            context.getSource().sendFeedback(message, false);
        }
        return 1;
    }
